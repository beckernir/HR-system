...
export default function UserProfilePage({ params }: ProfilePageProps) {
  ...
  useEffect(() => {
    ...
    console.log("User ID:", resolvedParams.userId);
    console.log("User ID type:", typeof resolvedParams.userId);
    console.log("User ID length:", resolvedParams.userId?.length);
    console.log("==================");
  }, [resolvedParams]);
  ...
  useEffect(() => {
    const validateUserId = async () => {
      try {
        console.log("Validating userId:", resolvedParams.userId);
        ...
        // Test API call directly here
        console.log("Testing API call for userId:", resolvedParams.userId);
        try {
          const testResponse = await apiService.getUserProfile(resolvedParams.userId);
  ...
  if (isValidating) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Loading profile for user: {resolvedParams.userId}</p>
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-4">User ID: {resolvedParams.userId}</p>
          <button
            onClick={() => router.back()}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors mr-2"
          >
      ...
      <div className="bg-yellow-100 border-b border-yellow-200 p-2">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-sm text-yellow-800">
            <strong>DEBUG:</strong> Loading profile for userId: <code>{resolvedParams.userId}</code>
            {" "}(Type: {typeof resolvedParams.userId}) - Check console for more details
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-2 py-3 text-sm text-gray-500">
            ...
            <span>/</span>
            <span className="text-gray-900">User Profile ({resolvedParams.userId})</span>
          </div>
        </div>
      </div>
