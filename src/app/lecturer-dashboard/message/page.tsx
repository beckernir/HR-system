...
//   //     // Filter out current user from search results and ensure valid data
//   //     const filteredResults = searchData.filter(
//   //       (user: User) => user && user.id && user.id !== currentUser?.id
//   //     );
...
//   //   }
//   // };
//   // Fixed fetchUsers function with proper error handling and validation
//   const fetchUsers = async () => {
//     try {
//       const usersData = await apiService.getAllUsers();

//       // Debug: Check what we actually received
//       console.log("Raw usersData:", usersData, "Type:", typeof usersData);
...
//       // Filter out invalid user data with additional safety checks
//       const validUsers = userArray.filter(
//         (user) => user && user.id && user.name && typeof user.name === "string"
//       );
...
//   // Fixed searchUsers function with the same protection
//   const searchUsers = async (query: string) => {
//     if (!query.trim()) {
//       setSearchResults([]);
//       return;
...
//         userArray = searchData;
//       } else if (searchData && Array.isArray(searchData.users)) {
//         userArray = searchData.users;
//       } else if (searchData && Array.isArray(searchData.data)) {
//         userArray = searchData.data;
//       } else {
//         console.warn("Unexpected search API response format:", searchData);
//         userArray = [];
//       }

//       // Filter out current user from search results and ensure valid data
//       const filteredResults = userArray.filter(
//         (user) => user && user.id && user.id !== currentUser?.id && user.name
//       );
...
//       // Transform conversations to chat format with validation
//       const chatList: Chat[] = conversations
//         .filter(
//           (conv: any) =>
//             conv && conv.otherUser && conv.otherUser.id && conv.otherUser.name
//         )
//         .map((conv: any) => ({
...
//   // Filter chats based on search with proper validation
//   const filteredChats = chats.filter((chat) => {
//     // Ensure chat and chat.user exist and have valid data
//     if (!chat || !chat.user || !chat.user.name) {
//       return false;
//     }
...
//                   <div className="flex items-center gap-3">
//                     <img
//                       src={selectedChat.user.avatar || "/Male.png"}
//                       alt="user"
//                       className="w-12 h-12 rounded-full object-cover"
//                     />
//                     <div>
